...
export BASETAG ?= builder-debian-sid-1.3.1
export SRC_BASE ?= /usr/local/src
export CONFIGURE_OPTIONS ?= --prefix=/usr --enable-static-executables=yes --enable-python
export DOLLAR := $$
SHELL := /bin/bash
...
SUBDIRS += libuna libfwnt libewf libewf_legacy libpff
...
libuna/Dockerfile:        export LOCAL_LIBS=libcdatetime libcerror libclocale libcnotify:master libcfile
...
libewf_legacy/Dockerfile: export LOCAL_LIBS=libbfio libcaes libcdata libcdatetime libcerror libcfile libclocale libcnotify libcpath libcsplit libcthreads libfcache libfdata libfguid libfvalue libhmac libodraw libsmdev libsmraw libuna libcstring libcsystem
libpff/Dockerfile:			  export LOCAL_LIBS=libbfio libcdata libcerror libcfile libclocale libcnotify libcpath libcsplit libcthreads libfcache libfdata libfdatetime libfguid libfmapi libfvalue libfwnt libuna

#USE_LN uses checkout + ln
#USE_COPY uses docker COPY
#USE_SYNC uses synclibs.sh, which uses clone + cp
export USE_LN=false
export USE_COPY=false
export USE_SYNC=true
